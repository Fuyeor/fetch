...
/** Fetches all domain belonging to the authenticated user. */
export async function fetchDomains(): Promise<Domain[]> {
  return await apiClient.get<Domain[]>('/domain');
}
...
export async function createDomain(payload: AddDomainPayload): Promise<Domain> {
  return await apiClient.post<Domain>('/domain', payload);
}
...
export async function verifyDomainOwnership(
  ...
  return await apiClient.post<VerifyDomainResult>(
    `/domain/${encodeURIComponent(domain)}/verify`,
  );
}

/** Deletes and unbinds a domain. */
export async function removeDomain(domain: string): Promise<void> {
  await apiClient.delete(`/domain/${encodeURIComponent(domain)}`);
}
